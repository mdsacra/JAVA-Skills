...
public class BooglanVerbsCalculator {
    ...
    public static void main(String[] args){
        ...
        final List<String> letrasFoo = new ArrayList<String>(Arrays.asList("r", "t", "c", "d", "b"));


        List<String> textoList = new ArrayList<String>(Arrays.asList(texto.split(" ")));

        List<String> verbos = new ArrayList<>();
        List<String> verbosNaPrimeira = new ArrayList<>();


        textoList.stream()
                .filter(palavra -> palavra.length() >= 8)
                .forEach(verbo -> {
                    int cont = 0;
                    for (String letraFoo: letrasFoo){
                        if (verbo.endsWith(letraFoo)){
                            cont++;
                        }
                    }
                    if (cont == 0){
        ...
        verbos.stream().forEach(verbo -> {
            int cont = 0;
            for (String letraFoo: letrasFoo){
                if (verbo.startsWith(letraFoo)){
                    cont++;
                }
            }
            if (cont == 0){
                verbosNaPrimeira.add(verbo);
            }
        });

        System.out.println(verbos.size());
        System.out.println(verbosNaPrimeira.size());
    }
}
