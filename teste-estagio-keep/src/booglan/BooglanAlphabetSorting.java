...
public class BooglanAlphabetSorting {

    public static void main(String[] args) {

        final String alfabeto = "twhzkdfvcjxlrnqmgpsb";
        ...
        String[] textoList2 = texto.split(" ");

        List<String> textoListFinal = new ArrayList<>();

        for (int i = 1; i < textoList2.length; i++){
            for (int j = 1; j < textoList2.length; j++){
                for (int k = 0; k < textoList2[j-1].length() && k < textoList2[j].length(); k++){
                    if (k == 0){
                        if (alfabeto.indexOf(textoList2[j-1].charAt(k)) > alfabeto.indexOf(textoList2[j].charAt(k))){
                            String palavraTroca = textoList2[j-1];
                            textoList2[j-1] = textoList2[j];
                            textoList2[j] = palavraTroca;
                            break;
                        }
                    } else {
                        if (alfabeto.indexOf(textoList2[j-1].charAt(k)) > alfabeto.indexOf(textoList2[j].charAt(k))){
                            if (alfabeto.indexOf(textoList2[j-1].charAt(k-1)) >= alfabeto.indexOf(textoList2[j].charAt(k - 1))) {
                                String palavraTroca = textoList2[j-1];
                                textoList2[j-1] = textoList2[j];
                                textoList2[j] = palavraTroca;
                            }
        ...
        for (String palavra: textoList2){
            if (!textoListFinal.contains(palavra)){
                textoListFinal.add(palavra);
            }
        }

        System.out.println(String.join(" ", textoListFinal));
    }
}
