...
public class BooglanBeautifulNumbers {
    ...
    public static void main(String[] args){
        ...
        String[] textoList = texto.split(" ");

        List<Long> beautyNumbers = new ArrayList<>();

        for (int j = 0; j < textoList.length; j++){
            long numero = 0;
            long mult = 1;
            for (int i = 0; i < textoList[j].length(); i++ ){
                if (i == 0) {
                    numero += alfabeto.indexOf(textoList[j].charAt(i));
                } else {
                    mult = mult * 20;
                    numero += (alfabeto.indexOf(textoList[j].charAt(i)))*mult;
                }
            }
            if (numero >= 422224 && numero % 3 == 0 && !(beautyNumbers.contains(numero))){
                beautyNumbers.add(numero);
            }
        }

        System.out.println(beautyNumbers.size());
    }
}
